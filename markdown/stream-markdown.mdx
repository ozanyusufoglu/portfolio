...
For optimal performance and control, consider transforming the streamed Markdown content into Tiptap‑compatible HTML or JSON format on the backend or frontend before inserting it into the editor using `streamContent` [^42], [^45], [^46], [^47], [^14]. This pre‑processing step can reduce the load on the editor by providing data in a format it handles more efficiently. When integrating with Vercel AI SDK or similar streaming services, an API route in Next.js can be set up to handle the AI response as a stream, and Tiptap's streaming API can then be connected to this route.
